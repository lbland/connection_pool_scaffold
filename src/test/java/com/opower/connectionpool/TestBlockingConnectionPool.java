...
public class TestBlockingConnectionPool {
	...
	public void setupSuccess(Integer count) throws SQLException {
		...
		for (Integer i = 0; i < count; ++i) {
			expect(mockConnectionWrapper.getConnection()).andReturn(
					mockConnection);
	 ...
	 * 
	 * - This class takes a connection and releases it after sleeping for a
	 * small bit of time. This allows the calling thread to request a connection
	 * and be blocked until this class releases the connection.
	 */
	...
	public static class DelayReleaseConnection implements Runnable {
		...
		DelayReleaseConnection(BlockingConnectionPool pool,
	...
	public void testBlockingRequest() throws SQLException {
		...
		Thread delayReleaseThread = new Thread(new DelayReleaseConnection(pool,
				connection));
	...
	public void testTimeOutRequest() throws SQLException {
		...
		try {
			Connection connection2 = pool.getConnection(1000,
	...
	public void testTimeOut() throws SQLException {
		...
		try {
			Connection connection2 = pool.getConnection(100,
					TimeUnit.MILLISECONDS);
		} catch (SQLException ex) {
			assertEquals(ex.getMessage(), "No connections became available");
